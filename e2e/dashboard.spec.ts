...
test.describe('Dashboard', () => {
  ...
  test.describe('Enrolled Courses', () => {
    test('should display enrolled courses section', async ({ page }) => {
      ...
      await expect(page.locator('text=/kursus.*saya|kursus.*terdaftar|enrolled/i')).toBeVisible();
    });
    ...
    test('should show enrolled course after enrollment', async ({ page }) => {
      ...
      if (courseTitle) {
        ...
        const enrolledSection = await page.locator('text=/kursus.*saya|enrolled/i').isVisible();
    ...
    test('should show empty state when no enrollments', async ({ page }) => {
      ...
      const emptyMessage = await page.locator('text=/belum.*terdaftar|no.*enrolled|mulai.*belajar/i').isVisible();
  ...
  test.describe('Progress Display', () => {
    test('should display overall progress', async ({ page }) => {
      ...
      const hasStats = await page.locator('[class*="stat" i], [class*="card" i]').count() > 0;
    ...
    test('should show course progress bars', async ({ page }) => {
      ...
      const hasProgress = await page.locator('[role="progressbar"], progress, [class*="progress" i]').count() > 0;
    ...
    test('should update progress after completing lesson', async ({ page }) => {
      ...
      const progressText = await page.locator('text=/\\d+.*%|\\d+.*selesai|completed/i').textContent();
  ...
  test.describe('Navigation', () => {
    ...
    test('should have link to browse more courses', async ({ page }) => {
      ...
      // Should have link to courses
      const browseCourses = page.locator('a[href="/courses"], text=/jelajah.*kursus|browse.*course/i');
  ...
  test.describe('Continue Learning', () => {
    test('should show continue learning button', async ({ page }) => {
      ...
      const continueButton = page.locator('text=/lanjutkan|continue|mulai/i').first();
