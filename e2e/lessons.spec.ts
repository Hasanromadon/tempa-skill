  ...
  test.describe("Lesson Page Access", () => {
    test("should display lesson page for enrolled user", async ({ page }) => {
      ...
      if (await firstCourse.isVisible()) {
        ...
        const firstLesson = page
          .locator('a[href*="/lessons/"], div:has-text("1.")')
          .first();
    ...
    test("should show lesson sidebar with all lessons", async ({ page }) => {
      ...
      if (await firstCourse.isVisible()) {
        ...
        if (await firstLesson.isVisible()) {
          ...
          await expect(
            page.locator("text=/daftar.*pelajaran|lesson.*list/i")
          ).toBeVisible();
        }
      }
    });
    ...
    test("should redirect to login for unauthenticated user", async ({
      ...
      const hasError = await page
        .locator("text=/akses|access|login/i")
        .isVisible();
      ...
      if (await firstCourse.isVisible()) {
        ...
        if (await firstLesson.isVisible()) {
          ...
          const nextButton = page
            .locator('button:has-text("Selanjutnya"), button:has-text("Next")')
            .first();
      ...
      if (await firstCourse.isVisible()) {
        ...
        if (await firstLesson.isVisible()) {
          ...
          const completeButton = page
            .locator(
              'button:has-text("Tandai Selesai"), button:has-text("Mark Complete")'
            )
            .first();
          if (await completeButton.isVisible()) {
            ...
            const completedBadge = await page
              .locator("text=/selesai|completed/i")
              .isVisible();
        ...
        if (await firstLesson.isVisible()) {
          ...
          if (await completeButton.isVisible()) {
            ...
            if (await sidebar.isVisible()) {
              ...
              const hasCheckmark =
                (await sidebar.locator("svg, text=/✓|selesai/i").count()) > 0;
      ...
      if (await firstCourse.isVisible()) {
        ...
        if (await firstLesson.isVisible()) {
          ...
          const progressText = await page
            .locator("text=/\\d+.*dari.*\\d+.*pelajaran/i")
            .textContent();
      ...
      if (await firstCourse.isVisible()) {
        ...
        if (await firstLesson.isVisible()) {
          ...
          const menuButton = page
            .locator(
              'button:has([class*="menu" i]), button[aria-label*="menu" i]'
            )
            .first();
